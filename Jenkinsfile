...
//                 print 'DB_PASS.collect { it }=' + DB_PASS.collect { it }
                sh 'docker run -d --rm --network bridge -p ${PORT}:${PORT} -e DB_USER=${DB_USER} -e DB_PASS=${DB_PASS} -e DB_URL=${DB_URL} -e SPRING_PROFILES_ACTIVE=prod --name ${CONTAINER_NAME} ${IMAGE_TAG}'
            }
        }
    }
}
